<nav x-data="{ active: 'beranda', open: false }" @scroll.window="
    ...
    <div class="max-w-8xl mx-auto px-4 sm:px-6 lg:px-10">
        <div class="h-20 flex items-center justify-between">
            <!-- Logo -->
            <img src="{{ asset('assets/images/logo-simak.svg') }}" alt="SIMAK" class="h-10 w-auto sm:h-14 md:h-16">
            ...
            <ul class="hidden md:flex items-center gap-12 font-medium">
                ...
                <li>
                    <a href="#alur-program" @click="open = false"
                ...
                <li>
                    <a href="#tanya-simak" @click="open = false"
    ...
    <!-- Overlay -->
    <div x-show="open" x-transition.opacity @click="open = false" class="fixed inset-0 z-40 bg-black/40 md:hidden"
    ...
    <aside x-show="open" x-transition:enter="transform transition ease-out duration-300"
        ...
        class="fixed right-0 top-0 z-50 h-screen w-1/2 bg-white shadow-2xl md:hidden" style="display: none;">
        <div class="flex h-20 items-center justify-between border-b px-5">
            <span class="font-semibold text-[#7A203A]">Menu</span>

            <button @click="open = false" class="rounded-lg p-2 hover:bg-gray-100" aria-label="Close menu">
